class App {
    constructor() {
        ...
        this.sampledImage = document.getElementById('sampled-image');
    ...
    handleToken(token) {
        ...
        this.outputContainer.className = this.currentTokens.length == 16 * 16 ? 'done' : 'loading';
        if (this.currentTokens.length % (16 * 4) == 0) {
            this.sampledImage.src = '/render?tokens=' + this.currentTokens.join(',');
        }
        this.updateProgress();
    }

    updateProgress() {
        this.progress.style.width = (100 * this.currentTokens.length / (16 * 16)).toFixed(2) + '%';
    }
}
